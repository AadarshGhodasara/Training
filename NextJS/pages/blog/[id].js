...
import postData from "../../component/data.json";
    ...
    <div className={styles.container}>
      ...
      <div className={styles.main}>
        <div className={styles.userTag}>
          <div className={styles.userImageTag}>
            <Image
          ...
          <div className={styles.userDetailsTag}>
            <h1>Blog {router?.query?.id}</h1>
            <h3>User Id : {post?.user}</h3>
            <h3>User Name : {post?.name}</h3>
            <h3>User Age : {post?.age}</h3>
            <h3>User PH : {post?.ph}</h3>
          </div>
        </div>
        <div className={styles.btnTag}>
          <button
            disabled={String(router?.query?.id) === "1"}
            onClick={() =>
              router.push(`/blog/${Number(router?.query?.id) - 1}`)
            }
            className={
              String(router?.query?.id) === "1"
                ? styles.disableBtn
                : styles.activeBtn
            }
          >
          ...
          <button
            disabled={Number(router?.query?.id) === postData.length}
            onClick={() =>
              router.push(`/blog/${Number(router?.query?.id) + 1}`)
            }
            className={
              String(router?.query?.id) === String(postData.length)
                ? styles.disableBtn
                : styles.activeBtn
            }
          >
...
id.getInitialProps = ({ query }) => {
  if (postData.length < query.id) {
    return {
      notFound: true,
    };
  }
  return {
    post: postData[query.id - 1],
  };
};
